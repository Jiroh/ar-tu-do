...
from rviz_geometry import show_circle_in_rviz, show_points_in_rviz, show_line_in_rviz, delete_marker
...
"""
Return true if all points are on one side of the line between point_a and point_b.
The parameter demanded_side indicates on which side all points should be.
demanded_side == 1: right side
demanded_side == -1: left side
"""
...
def calc_smallest_distance_to_points(point_a, point_b, points):
...
def calc_target_car_position(predicted_car_position, curve_type, left_circle, right_circle, upper_circle, left_wall, right_wall, remaining_distance, curve_entry_point):
    ...
    if left_circle.center.x < 0 and right_circle.center.x < 0 and (left_circle.radius < 10 or right_circle.radius < 10): #calc_smallest_distance_to_points([0, 0], [target_position.x, target_position.y], left_wall) < CAR_WIDTH
        left_tangent_point = calc_polygon_tangent_point(left_wall, -1)
        if left_tangent_point is not None:
            target_position = calc_orthogonal_point([0, 0], left_tangent_point, -1, 2.1)
    if left_circle.center.x > 0 and right_circle.center.x > 0 and (left_circle.radius < 10 or right_circle.radius < 10): #calc_smallest_distance_to_points([0, 0], [target_position.x, target_position.y], right_wall) < CAR_WIDTH
        right_tangent_point = calc_polygon_tangent_point(right_wall, 1)
        if right_tangent_point is not None:
            target_position = calc_orthogonal_point([0, 0], right_tangent_point, 1, 2.1)
...
def follow_walls(left_circle, right_circle, upper_circle, left_wall, right_wall, curve_type, remaining_distance, curve_entry_point, delta_time):
    global last_speed
    predicted_car_position, prediction_distance = calc_predicted_car_position(upper_circle is None)
    target_position = calc_target_car_position(predicted_car_position, curve_type, left_circle, right_circle, upper_circle, left_wall, right_wall, remaining_distance, curve_entry_point)
...
def handle_scan(laser_scan, delta_time):
    ...
    follow_walls(left_circle, right_circle, upper_circle, left_wall, right_wall, curve_type, remaining_distance, curve_entry_point, delta_time)
